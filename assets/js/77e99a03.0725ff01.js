...
`,i="https://esm.sh",s=["/App.tsx","/App.js","/App.jsx","/App.ts"],a=["",".tsx",".ts",".jsx",".js"],o=e=>/\.[jt]sx?$/.test(e),l=/(\bfrom\s*|\bimport\s*|\bimport\(\s*)(['"])(\.[^'"]*)\2/g,d=/^[^\S\n]*import\s+(['"])([^'"]+\.s?css)\1;?[^\S\n]*$/gm;class c extends Error{file;constructor(e,r){super(e),this.file=r,this.name="CompileError"}}let u=(e,r,t="")=>{let n=r[e];return`${i}/${e}${n?`@${encodeURIComponent(n)}`:""}${t}`},h=(e,r,t)=>{let n=r.slice(0,r.lastIndexOf("/")+1),i=new URL(e,`file://${n}`).pathname;for(let e of a)if(t[i+e])return i+e;throw new c(`Cannot resolve '${e}' from '${r}'`,r)},p=String.raw`
...
post('mounted');
`,m=e=>e.replaceAll("</",String.raw`<\/`),f=(e,r,t)=>{let a=s.find(r=>e[r]);if(!a)throw new c("No entry module. Expected /App.tsx or /App.js.");let f=((e,r)=>{let t={};for(let n of Object.values(e)){let i;if(!o(n.path))continue;let s=n.code.replaceAll(d,"");try{i=r(s,{transforms:["typescript","jsx"],jsxRuntime:"automatic",filePath:n.path}).code}catch(r){let{message:e}=r;throw new c(`${n.path}: ${e}`,n.path)}let a=[],u=i.replaceAll(l,(r,t,i,s)=>{let o=h(s,n.path,e);return a.push(o),`${t}${i}__RQBMOD:${o}__${i}`});t[n.path]={code:u,deps:a}}return t})(e,r),y=(e=>{let r=e.react??"18",t=e["react-dom"]??r,n={react:`${i}/react@${encodeURIComponent(r)}`,"react/":`${i}/react@${encodeURIComponent(r)}/`,"react-dom":`${i}/react-dom@${encodeURIComponent(t)}?external=react`,"react-dom/":`${i}/react-dom@${encodeURIComponent(t)}&external=react/`};for(let r of Object.keys(e))"react"!==r&&"react-dom"!==r&&(n[r]=u(r,e,"?external=react,react-dom"));return{imports:n}})(t.dependencies),g=((e,r)=>{let t=new Set(r.extraCSSImports);for(let r of Object.values(e))if(o(r.path))for(let[,,e]of r.code.matchAll(d))e.startsWith(".")||t.add(e);return[...t].map(e=>((e,r)=>{let t,n,[i,s]=(t=e.split("/"),[n=e.startsWith("@")?t.slice(0,2).join("/"):t[0],e.slice(n.length+1)]);return u(i,r,`/${s}`).replace(/\.scss$/,".css")})(e,r.dependencies))})(e,t),b=[n,...Object.values(e).filter(e=>{let r;return r=e.path,/\.s?css$/.test(r)}).map(e=>e.code)].filter(Boolean).join("\n\n");return`<!doctype html>
<html>
<head>
<meta charset="utf-8">
