...
`expressionParserCEL` is the ready-to-use handler for [`parseCEL`](/docs/utils/import.md#cel), bound to the built-in CEL inverse. It handles arithmetic, function-call, and `min`/`max` template operands:
...
`expressionParserSpEL` is the ready-to-use handler for [`parseSpEL`](/docs/utils/import.md#spel), bound to the built-in SpEL inverse. It handles infix arithmetic operands:
